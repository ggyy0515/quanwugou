...
@interface ECPointMallViewController ()
<
...
@implementation ECPointMallViewController
...
- (void)loadDataSource {
    [ECHTTPServer requestPointProductListWithPageNumber:_pageNum
                                                succeed:^(NSURLSessionDataTask *task, id result) {
                                                    ...
                                                    if (IS_REQUEST_SUCCEED(result)) {
                                                        ...
                                                        for (NSDictionary *dic in result[@"integralList"]) {
                                                            ECPointProductListModel *model = [ECPointProductListModel yy_modelWithDictionary:dic];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ECPointProductListModel *model = [_dataSource objectAtIndexWithCheck:indexPath.row];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ECPointProductListModel *model = [_dataSource objectAtIndexWithCheck:indexPath.row];
    ECPointProductDetailViewController *vc = [[ECPointProductDetailViewController alloc] initWithModel:model];
    [SELF_BASENAVI pushViewController:vc animated:YES titleLabel:@"商品详情"];
}
