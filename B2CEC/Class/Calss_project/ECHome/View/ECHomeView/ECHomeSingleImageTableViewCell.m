...
@interface ECHomeSingleImageTableViewCell()
...
@implementation ECHomeSingleImageTableViewCell

+(instancetype)cellWithTableView:(UITableView *)tableView{
    ECHomeSingleImageTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CELL_IDENTIFY_WITH_OBJECT(ECHomeSingleImageTableViewCell)];
    if (cell == nil) {
        cell = [[ECHomeSingleImageTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CELL_IDENTIFY_WITH_OBJECT(ECHomeSingleImageTableViewCell)];
...
- (void)createBasicUI{
    ...
    _iconImageView.image = [UIImage imageNamed:@"placeholder_News2"];
    ...
    [_titleLab mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.mas_equalTo(weakSelf.iconImageView.mas_left).offset(-16.f);
    }];
    ...
    [_iconImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(110.f, 72.f));
...
- (void)setModel:(ECHomeNewsListModel *)model{
    ...
    [_iconImageView yy_setImageWithURL:[NSURL URLWithString:[IMAGEURL(model.cover1) stringByReplacingOccurrencesOfString:@"/res" withString:@""]] placeholder:[UIImage imageNamed:@"placeholder_News2"]];
}
