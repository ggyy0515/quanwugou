...
- (instancetype)initWithFrame:(CGRect)frame collectionViewLayout:(nonnull UICollectionViewLayout *)layout
{
    ...
    if (self) {
        ...
        [self addGesture];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handelCellClick:) name:NotificationNewsChangeState object:nil];
...
- (void)gestureBegan:(UILongPressGestureRecognizer *)longPressGesture{
    ...
    if (_originalIndexPath.section == 0 && _originalIndexPath.row != 0) {
        //获取手指所在的cell
...
- (void)gestureChange:(UILongPressGestureRecognizer *)longPressGesture{
    ...
    if (point.x < 130.f && point.y < 110.f) {
        return;
    }
    //通知代理
...
- (void)handelCellClick:(NSNotification *)notification {
    ECHomeAllOptionCollectionViewCell *cell = notification.object;
    ...
    if (indexPath.section == 0) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            if (self.removeCellBlock) {
                self.removeCellBlock(indexPath);
            }
        }];
    }else{
        [UIView animateWithDuration:0.2 animations:^{
...
- (void)setBeginEditing:(BOOL)beginEditing {
    ...
    if (beginEditing) {
        [[NSNotificationCenter defaultCenter] postNotificationName:NotificationNewsBeginEditing object:@"yes"];
    }else {
        [[NSNotificationCenter defaultCenter] postNotificationName:NotificationNewsBeginEditing object:@"no"];
    }
}
