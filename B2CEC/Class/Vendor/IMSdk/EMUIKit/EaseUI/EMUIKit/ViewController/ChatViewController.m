...
@interface ChatViewController ()<UIAlertViewDelegate, EaseMessageViewControllerDelegate, EaseMessageViewControllerDataSource,EMClientDelegate,EaseMessageCellDelegate>
{
...
- (void)viewDidLoad {
    ...
    [(MJRefreshNormalHeader *)self.tableView.mj_header setTitle:@"下拉加载历史记录" forState:MJRefreshStateIdle];
    [(MJRefreshNormalHeader *)self.tableView.mj_header setTitle:@"松开即可加载" forState:MJRefreshStatePulling];
    [(MJRefreshNormalHeader *)self.tableView.mj_header setTitle:@"正在加载中" forState:MJRefreshStateRefreshing];
    [(MJRefreshNormalHeader *)self.tableView.mj_header setTitle:@"即将为您加载" forState:MJRefreshStateWillRefresh];
    ...
    //判断是否需要发送一段话
//    if (self.detailModel) {
//        [self sendTextMessage:[NSString stringWithFormat:@"嗨，%@，[%@]复制店名到商家查找。[%@]复制账号去加为好友。",((CMBaseNavigationController *)self.navigationController).titleLabel.text,self.detailModel.name,self.detailModel.username]];
//    }
}
...
- (void)_setupBarButtonItem
{
    ...
    if (self.conversation.type == EMConversationTypeChat) {
        ...
        [clearButton setImage:[[UIImage imageNamed:@"delete_em"] imageWithColor:DarkMoreColor] forState:UIControlStateNormal];
                           ...
                           modelForMessage:(EMMessage *)message
{
    ...
    model.avatarImage = [UIImage imageNamed:@"face1"];
    model.failImageName = @"imageDownloadFail";
    if (model.isSender) {
        model.avatarURLPath = IMAGEURL([USERDEFAULT objectForKey:EC_USER_HEAD_IMAGE]);
        model.nickname = [USERDEFAULT objectForKey:EC_USER_NICKNAME];
    } else {
        model.nickname = @"全屋构客服";
        if (_chatListModel) {
            model.nickname = _chatListModel.name;
            model.avatarURLPath = IMAGEURL(_chatListModel.headImage);
        }
    }
    return model;
}
...
//    if ([self.friendModel.isbiz1 isEqualToString:@"0"]) {
//        ECMerchantsHomePageTBVC *MerchantsHomePageTBVC = [[ECMerchantsHomePageTBVC alloc]init];
//        MerchantsHomePageTBVC.userinfoids = _friendModel.userinfoids2;
//        [(CMBaseNavigationController *)self.navigationController pushViewController:MerchantsHomePageTBVC animated:YES titleLabel:@"个人主页"];
//    }else{
//        ECMerchantsHPageTBVC *MerchantsHPageTBVC = [[ECMerchantsHPageTBVC alloc]init];
//        MerchantsHPageTBVC.userinfoids = _friendModel.userinfoids2;
//        [(CMBaseNavigationController *)self.navigationController pushViewController:MerchantsHPageTBVC animated:YES titleLabel:@"商户主页"];
//    }
}
...
- (void)deleteAllMessages:(id)sender{
    ...
    if ([sender isKindOfClass:[NSNotification class]]) {
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"是否删除聊天记录" message:nil delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"好的", nil];
        [alertView show];
    }
}
